...
BOOL CNtfsundelDlg::OnInitDialog()
{
	...
	m_files.SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES | LVS_EX_HEADERDRAGDROP);   
	...
	else if (*pattern == _T('*'))
	{
		...
		else
		{
			if (*pattern)
			{
				...
				if (*fileName == *pattern && *fileName == _T('\0') && (fileName-ff) >= (pattern-pp))
					return TRUE;
...
void CNtfsundelDlg::OnSearch() 
{
	...
	for (ULONGLONG i = MFT_IDX_USER; i < volume.GetRecordsCount(); i ++)
	{
		...
		if (MatchFileName((const _TCHAR*)fns, (const _TCHAR*)filters))
		{
			...
			s.Format(_T("%04d-%02d-%02d  %02d:%02d"), st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute);
			...
			if (count > 50000)
			{
				MessageBox(_T("Too many files found, only the first 50,000 will be shown"));
				break;
			}
		}
...
void CNtfsundelDlg::OnRecover() 
{
	...
	if (savedlg.DoModal() == IDOK)
	{
		...
		{
			if (MessageBox(_T("You should choose a different drive, do you want to continue anyway ?"),
				NULL, MB_OKCANCEL) == IDCANCEL)
				return;
		}

		HANDLE hf = CreateFile((const _TCHAR*)path, GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
		...
		if (data)
		{
			...
			{
				if (MessageBox(_T("File size exceeds 100M, do you want to continue anyway ?"),
					NULL, MB_OKCANCEL) == IDCANCEL)
					return;
			}
			...
			for (ULONGLONG i = 0; i < datalen; i += BUFSIZE)
			{
				...
				DWORD len;
				if (data->ReadData(i, buf, BUFSIZE, &len) && (len == BUFSIZE || len == remain))
				{
					// Save data
					DWORD l;
...
void CNtfsundelDlg::OnSelchangeDriver() 
{
	m_files.DeleteAllItems();
}
