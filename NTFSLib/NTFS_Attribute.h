...
__inline DWORD CAttrBase::GetAttrType() const { return AttrHeader->Type; }
...
__inline WORD CAttrBase::GetAttrFlags() const { return AttrHeader->Flags; }
...
CAttrResident::CAttrResident(const ATTR_HEADER_COMMON* ahc,
                             const CFileRecord* fr)
    : CAttrBase(ahc, fr)
{
...
BOOL CAttrResident::ReadData(const ULONGLONG& offset, void* bufv, DWORD bufLen,
                             DWORD* actural) const
{
  _ASSERT(bufv);
...
CAttrNonResident::CAttrNonResident(const ATTR_HEADER_COMMON* ahc,
                                   const CFileRecord* fr)
    : CAttrBase(ahc, fr)
{
...
BOOL CAttrNonResident::PickData(const BYTE** dataRun, LONGLONG* length,
                                LONGLONG* LCNOffset)
{
  BYTE size = **dataRun;
  (*dataRun)++;
...
__inline BOOL CAttrNonResident::IsDataRunOK() const { return bDataRunOK; }
...
// Read "bufLen" bytes from "offset" into "bufv"
// Number of bytes acturally read is returned in "*actural"
BOOL CAttrNonResident::ReadData(const ULONGLONG& offset, void* bufv,
...
CAttr_StdInfo::CAttr_StdInfo(const ATTR_HEADER_COMMON* ahc,
                             const CFileRecord* fr)
    : CAttrResident(ahc, fr)
{
...
CAttr_StdInfo::~CAttr_StdInfo() { NTFS_TRACE("CAttr_StdInfo deleted\n"); }

// Change from UTC time to local time
void CAttr_StdInfo::GetFileTime(FILETIME* writeTm, FILETIME* createTm,
...
int CFileName::GetFileName(char* buf, DWORD bufLen) const
{
  ...
  if (FileName->NameLength)
  {
    ...
    len = WideCharToMultiByte(CP_ACP, 0, (wchar_t*)FileName->Name,
                              FileName->NameLength, buf, bufLen, NULL, NULL);
    if (len)
    {
      buf[len] = '\0';
...
__inline BOOL CFileName::HasName() const { return FileNameLength > 0; }

__inline BOOL CFileName::IsWin32Name() const
{
  if (FileName == NULL || FileNameLength <= 0) return FALSE;

  return (FileName->NameSpace !=
          ATTR_FILENAME_NAMESPACE_DOS);  // POSIX, WIN32, WIN32_DOS
}

// Change from UTC time to local time
void CFileName::GetFileTime(FILETIME* writeTm, FILETIME* createTm,
                            FILETIME* accessTm) const
{
...
class CAttr_FileName : public CAttrResident, public CFileName
{
 public:
  ...
  virtual ~CAttr_FileName() { NTFS_TRACE("CAttr_FileName deleted\n"); }
...
class CAttr_VolInfo : public CAttrResident
{
 public:
  ...
  virtual ~CAttr_VolInfo() { NTFS_TRACE("CAttr_VolInfo deleted\n"); }
...
{
 public:
  CAttr_Data(const ATTR_HEADER_COMMON* ahc, const CFileRecord* fr)
      : TYPE_RESIDENT(ahc, fr)
  {
    NTFS_TRACE1("Attribute: Data (%sResident)\n", IsNonResident() ? "Non" : "");
  }

  virtual ~CAttr_Data() { NTFS_TRACE("CAttr_Data deleted\n"); }
};  // CAttr_Data
...
class CIndexEntry : public CFileName
{
 public:
  ...
  virtual ~CIndexEntry()
  {
    ...
    if (IsCopy && IndexEntry) delete (void*)IndexEntry;
...
class CIndexBlock : public CIndexEntryList
{
 public:
  ...
  virtual ~CIndexBlock()
  {
    ...
    if (IndexBlock) delete IndexBlock;
  }
...
CAttr_IndexRoot::CAttr_IndexRoot(const ATTR_HEADER_COMMON* ahc,
                                 const CFileRecord* fr)
    : CAttrResident(ahc, fr)
{
...
{
  INDEX_ENTRY* ie;
  ie = (INDEX_ENTRY*)((BYTE*)(&(IndexRoot->EntryOffset)) +
...
CAttr_IndexAlloc::CAttr_IndexAlloc(const ATTR_HEADER_COMMON* ahc,
                                   const CFileRecord* fr)
    : CAttrNonResident(ahc, fr)
{
...
BOOL CAttr_IndexAlloc::PatchUS(WORD* sector, int sectors, WORD usn,
                               WORD* usarray)
{
  int i;
...
BOOL CAttr_IndexAlloc::ParseIndexBlock(const ULONGLONG& vcn,
                                       CIndexBlock& ibClass)
{
...
template <class TYPE_RESIDENT>
CAttr_Bitmap<TYPE_RESIDENT>::CAttr_Bitmap(const ATTR_HEADER_COMMON* ahc,
                                          const CFileRecord* fr)
    : TYPE_RESIDENT(ahc, fr)
{
...
template <class TYPE_RESIDENT>
CAttr_AttrList<TYPE_RESIDENT>::CAttr_AttrList(const ATTR_HEADER_COMMON* ahc,
                                              const CFileRecord* fr)
    : TYPE_RESIDENT(ahc, fr)
{
         ...
         len == sizeof(ATTR_ATTRIBUTE_LIST))
  {
    ...
    if (recordRef != fr->FileReference)  // Skip contained attributes
    {
      ...
      if (am & fr->AttrMask)  // Skip unwanted attributes
      {
        ...
        const CAttrBase* ab =
            (CAttrBase*)frnew->FindFirstAttr(alRecord.AttrType);
        while (ab)
        {
          CAttrList* al =
              (CAttrList*)&fr->AttrList[ATTR_INDEX(alRecord.AttrType)];
