...
typedef void (*ATTR_RAW_CALLBACK)(const ATTR_HEADER_COMMON* attrHead,
                                  BOOL* bDiscard);
...
class CAttrBase;
typedef void (*ATTRS_CALLBACK)(const CAttrBase* attr, void* context,
                               BOOL* bStop);
...
{
 public:
  CEntrySmartPtr(ENTRY_TYPE* ptr = NULL) { EntryPtr = ptr; }

  virtual ~CEntrySmartPtr()
  {
    if (EntryPtr) delete EntryPtr;
  }
 ...
 public:
  ...
  __inline BOOL IsValid() const { return EntryPtr != NULL; }
};
...
class CSList
{
 ...
 public:
  // Get entry count
  __inline int GetCount() const { return EntryCount; }
...
class CStack
{
 ...
 public:
  ...
  BOOL Push(ENTRY_TYPE* entry)
  {
    ...
    if (ListTail == NULL) ListTail = le;  // Empty list
  ...
  ENTRY_TYPE* Pop()
  {
    ...
    if (ListTail == ListHead) ListTail = ListHead->Next;
