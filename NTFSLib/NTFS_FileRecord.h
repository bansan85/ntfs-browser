...
class CFileRecord
{
...
#else
  template <class TYPE_RESIDENT>
  friend class CAttr_AttrList;  // Won't compiler in VC6.0, why?
#endif
 ...
 private:
  ...
  __inline void UserCallBack(DWORD attType, ATTR_HEADER_COMMON* ahc,
                             BOOL* bDiscard);
 ...
 public:
  ...
  __inline const BOOL FindSubEntry(const _TCHAR* fileName,
                                   CIndexEntry& ieFound) const;
...
CFileRecord::~CFileRecord()
{
  ...
  if (FileRecord) delete FileRecord;
}
...
__inline void CFileRecord::UserCallBack(DWORD attType, ATTR_HEADER_COMMON* ahc,
                                        BOOL* bDiscard)
{
  *bDiscard = FALSE;
...
BOOL CFileRecord::VisitIndexBlock(const ULONGLONG& vcn, const _TCHAR* fileName,
                                  CIndexEntry& ieFound) const
{
  CAttr_IndexAlloc* ia =
      (CAttr_IndexAlloc*)FindFirstAttr(ATTR_TYPE_INDEX_ALLOCATION);
...
void CFileRecord::TraverseSubNode(const ULONGLONG& vcn,
                                  SUBENTRY_CALLBACK seCallBack) const
{
  CAttr_IndexAlloc* ia =
      (CAttr_IndexAlloc*)FindFirstAttr(ATTR_TYPE_INDEX_ALLOCATION);
...
BOOL CFileRecord::ParseAttrs()
{
  ...
  ATTR_HEADER_COMMON* ahc =
      (ATTR_HEADER_COMMON*)((BYTE*)FileRecord + FileRecord->OffsetOfAttr);
...
{
  for (int i = 0; i < ATTR_NUMS; i++) AttrRawCallBack[i] = NULL;
}
...
int CFileRecord::GetFileName(_TCHAR* buf, DWORD bufLen) const
{
  ...
  CAttr_FileName* fn =
      (CAttr_FileName*)AttrList[ATTR_INDEX(ATTR_TYPE_FILE_NAME)]
          .FindFirstEntry();
  while (fn)
  {
    if (fn->IsWin32Name())
    ...
    fn = (CAttr_FileName*)AttrList[ATTR_INDEX(ATTR_TYPE_FILE_NAME)]
             .FindNextEntry();
  }
...
// Get File Times
void CFileRecord::GetFileTime(FILETIME* writeTm, FILETIME* createTm,
                              FILETIME* accessTm) const
{
  // Standard Information attribute hold the most updated file time
  CAttr_StdInfo* si =
      (CAttr_StdInfo*)AttrList[ATTR_INDEX(ATTR_TYPE_STANDARD_INFORMATION)]
          .FindFirstEntry();
  if (si)
...
void CFileRecord::TraverseSubEntries(SUBENTRY_CALLBACK seCallBack) const
{
  ...
  if (ir == NULL || !ir->IsFileName()) return;
...
__inline const BOOL CFileRecord::FindSubEntry(const _TCHAR* fileName,
                                              CIndexEntry& ieFound) const
{
  // Start searching from IndexRoot (B+ tree root node)
  CAttr_IndexRoot* ir = (CAttr_IndexRoot*)FindFirstAttr(ATTR_TYPE_INDEX_ROOT);
  if (ir == NULL || !ir->IsFileName()) return FALSE;
...
CNTFSVolume::CNTFSVolume(_TCHAR volume)
{
  ...
  if (!vol.ParseFileRecord(MFT_IDX_VOLUME)) return;

  vol.ParseAttrs();
  CAttr_VolInfo* vi =
      (CAttr_VolInfo*)vol.FindFirstAttr(ATTR_TYPE_VOLUME_INFORMATION);
  if (!vi) return;
...
__inline BOOL CNTFSVolume::IsVolumeOK() const { return VolumeOK; }

// Get NTFS volume version
__inline WORD CNTFSVolume::GetVersion() const { return Version; }
...
__inline DWORD CNTFSVolume::GetSectorSize() const { return SectorSize; }

__inline DWORD CNTFSVolume::GetClusterSize() const { return ClusterSize; }

__inline DWORD CNTFSVolume::GetFileRecordSize() const { return FileRecordSize; }

__inline DWORD CNTFSVolume::GetIndexBlockSize() const { return IndexBlockSize; }

// Get MFT starting address
__inline ULONGLONG CNTFSVolume::GetMFTAddr() const { return MFTAddr; }
